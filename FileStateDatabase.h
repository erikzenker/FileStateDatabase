...
inline FileStateDatabase::FileStateDatabase(std::vector<std::string> ignoredDirectories, std::string dataBaseLocation, boost::filesystem::path rootPath) : 
  mRootPath(rootPath),
...
inline FileStateDatabase::~FileStateDatabase(){
  sqlite3_close(mDataBase);
 ...
 * Sqlite callback
 */
inline int FileStateDatabase::noAction(void *NotUsed, int argc, char **argv, char **azColName){
 ...
 * Sqlite callback
 */
inline int FileStateDatabase::getFileStates(void *fileStates, int argc, char **argv, char **azColName){
 ...
 *
 */
inline bool FileStateDatabase::executeQuery(const std::string query, int (*callback)(void*,int,char**,char**) ,void* userData){
  char *errorMsg = 0;
...
inline FileState FileStateDatabase::createFileState(boost::filesystem::path path){
  FileState fileState;
  struct stat buffer;
  fileState.path    = path;
...
inline void FileStateDatabase::createFileStateCache(){
...
inline bool FileStateDatabase::propagateUpdate(const boost::filesystem::path path,const ModState ms){
...
inline bool FileStateDatabase::propagateUpdate(const std::pair<FileState, ModState> update){
...
inline void FileStateDatabase::propagateUpdateRecursively(const boost::filesystem::path rootPath, const ModState ms){
  try {
    if(ms == FS_DELETE){
 ...
 *
 **/
inline bool FileStateDatabase::insertFileState(const FileState fileState){
 ...
 *
 **/
inline bool FileStateDatabase::updateFileState(const FileState fileState){
  std::stringstream query;
 ...
 *
 **/
inline bool FileStateDatabase::deleteFileState(const FileState fileState, const bool recursive){
  std::stringstream query;
  if(recursive){
 ...
 *
 **/
inline void FileStateDatabase::setFileState(const FileState fs){
 ...
 *
 **/
inline FileState FileStateDatabase::getFileState(const boost::filesystem::path path){
 ...
 *
 **/
inline std::vector<std::pair<FileState, ModState> > FileStateDatabase::updates(){
...
inline std::string FileStateDatabase::modStateToString(const ModState modState){
  switch(modState){
  case FS_CREATE:
...
inline bool FileStateDatabase::isIgnored( std::string file){
